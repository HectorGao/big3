...
test('empty session cannot be logged, partial session does not advance cycle',()=>{
 const {store}=setup();const d=store.load();d.profile=profile;store.save(d);const s=P.createSession(P.makePlan({profile,lift:'squat'}));assert.throws(()=>store.finish(s));s.sets[0].weight=10;s.sets[0].calibrationRequired=false;s.sets[0].done=true;const r=store.finish(s);assert.equal(r.completed,false);assert.equal(store.load().history.length,1);assert.throws(()=>store.finish(s),/已经保存/);
});
...
test('shortened main reps do not count as a completed prescribed session',()=>{
 const {store}=setup();const s=P.createSession(P.makePlan({profile,lift:'squat'}));s.sets.forEach(row=>{row.done=true;row.weight=10;row.calibrationRequired=false;});s.sets[0].reps=1;assert.equal(store.finish(s).completed,false);
});
...
test('cross-day archive uses conservative finish date for recovery',()=>{
 const {store}=setup();const s=P.createSession(P.makePlan({profile,lift:'squat'}));s.date='2026-09-01';s.sets[0].weight=10;s.sets[0].calibrationRequired=false;s.sets[0].done=true;store.save({...store.load(),profile,session:s});assert.equal(store.finish(s).date,P.dateKey());
});
