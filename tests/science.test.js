...
test('all 52 exercises declare independent stage media and measurement metadata',()=>{
 assert.equal(C.exercises.length,52);
...
test('all five readiness levels are explained and affect planning without auto-maxing',()=>{assert.equal(S.fatigueLevels.length,5);for(const l of S.fatigueLevels){assert.ok(l.label&&l.description&&l.action);}const get=level=>P.makePlan({profile,readiness:{fatigue:level}});assert.ok(get(1).exercises[0].weight>get(2).exercises[0].weight);assert.ok(get(3).exercises[0].weight<get(2).exercises[0].weight);assert.ok(get(3).exercises[0].sets<get(2).exercises[0].sets);assert.equal(get(4).mode,'recovery');assert.equal(get(5).mode,'rest');assert.throws(()=>get(6));});
