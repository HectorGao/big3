...
test('native mode/date selection and machine trial use shared prescriptions',()=>{
 ...
 Object.assign(today.data.trial,{weight:45,reps:12,rir:3,quality:true,machineId:'gym-A',increment:5});today.saveTrial();
...
test('native PB estimate, editable weight/reps, and per-set reserve share web calculations',()=>{
 ...
 const today=page('today',app);today.refresh();assert.equal(today.data.plan.exercises[0].weight,92.5);today.editPlan({currentTarget:{dataset:{index:0,field:'weight'}},detail:{value:'100'}});assert.equal(today.data.plan.exercises[0].reps,3);today.start();assert.equal(today.data.session.sets[0].weight,'100');assert.equal(today.data.session.sets[0].reps,3);today.setRir({currentTarget:{dataset:{index:0}},detail:{value:'4'}});assert.equal(app.store.load().session.sets[0].rir,3);
});
